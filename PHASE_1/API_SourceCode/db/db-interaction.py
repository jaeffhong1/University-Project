...
def format_date_for_db(timedate) -> tuple:
    ...
    try:
        ...
        date, time = timedate.split(' ') # there should be a space separating the two halves
        
        # extract the date data
        year, month, day = date.split('-') # year, month and date should be separated by hyphen ('-')
        
        # extract the time data
        # ignore seconds
        hour, minute, _ = time.split(':') # hour, minute and seconds should be separated by colon (':')
        
        # hour is a required value in the database
        hour_int = int(hour)
    except:
        raise ValueError(f"The format of parameter 'timedate' was wrong. Expected value of format of \"YYYY-M-D HH:MM:SS\", but recieved \"{timedate}\".")
...
def insert_article(article: dict) -> int:
    ...
    if minute == None:
        raise ValueError(f"Dictionary parameter 'article' contains insufficient datetime information in key 'date_of_publication' for insertion in Articles table. Ensure the 'date_of_publication' contains valid 'minute' data.")
