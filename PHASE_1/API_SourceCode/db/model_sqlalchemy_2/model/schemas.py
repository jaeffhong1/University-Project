...
from sqlalchemy import Table, Column, Integer, String, Date, ForeignKey, PrimaryKeyConstraint
...
class Disease(Base):
    ...
    id = Column('id', Integer, nullable=False, unique=True, autoincrement=True, primary_key=True)
    name = Column('name', String, nullable=False, unique=True)
    
    reportdisease = relationship('ReportDisease', back_populates='disease', uselist=False)
...
class Syndrome(Base):
    ...
    reportsyndrome = relationship('ReportSyndrome', back_populates='syndrome', uselist=False)
...
class ReportDisease(Base):
    ...
    id = Column('id', Integer, nullable=False, unique=True, autoincrement=True, primary_key=True)
    disease_id = Column('disease_id', Integer, ForeignKey('Diseases.id'), nullable=False)
...
class EventDate(Base):
    ...
    report_start = relationship('Report', back_populates='start_eventdate', foreign_keys='Report.start_eventdate_id')
    report_finish = relationship('Report', back_populates='finish_eventdate', foreign_keys='Report.finish_eventdate_id')
...
class Article(Base):
    ...
    def __init__(self, url, headline, eventdate=None, eventdate_id=None):
        ...
        if eventdate == None and eventdate_id == None:
            ...
            self.eventdate = EventDate(id=eventdate_id) # get the event date with the id
...
class Report(Base):
    ...
    start_eventdate_id = Column('start_eventdate_id', ForeignKey('EventDates.id'), nullable=False)
    finish_eventdate_id = Column('finish_eventdate_id', ForeignKey('EventDates.id'), nullable=True)
    
    # to
    article = relationship('Article', back_populates='report', uselist=False)
    start_eventdate = relationship('EventDate', back_populates='report_start', uselist=False, foreign_keys='Report.start_eventdate_id')
    finish_eventdate = relationship('EventDate', back_populates='report_finish', uselist=False, foreign_keys='Report.finish_eventdate_id')
    ...
    def __init__(self, article, start_eventdate, finish_eventdate, reportlocations, diseases, syndromes):
        ...
        elif type(diseases) == list and len(diseases) > 0 and type(diseases[0]) == Disease:
            self.diseases = diseases
        elif type(diseases) == list and len(diseases) == 0:
            self.syndromes = []
        else:
            raise ValueError(f"diseases must be a list of disease names or Disease objects but recieved {diseases} of type {type(diseases)}.")
        ...
        elif type(syndromes) == list and len(syndromes) > 0 and type(syndromes[0]) == Syndrome:
            self.syndromes = syndromes
        elif type(syndromes) == list and len(syndromes) == 0:
            self.syndromes = []
        else:
            raise ValueError(f"syndromes must be a list of disease names or Syndrome objects but recieved {syndromes} of type {type(syndromes)}.")
